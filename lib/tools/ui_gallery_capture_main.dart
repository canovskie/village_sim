...
ResourceBundle _stock() => ResourceBundle(
    wood: 62, stone: 38, iron: 12, coal: 9, food: 74, honey: 5, reed: 11, gold: 26);
...
VillagerEntity _mkVillager(String name, String surname, VillagerType t, double age,
    {int seed = 7}) {
...
VillagerEntity _richVillager() {
  final v = _mkVillager('Ayşe', 'Demirhan', VillagerType.merchant, 6.5, seed: 11);
  v.morale = 0.72;
  v.mood = 0.3;
  v.isFavorite = true;
  v.wed = true;
  final esi = _mkVillager('Kemal', 'Demirhan', VillagerType.blacksmith, 7.0, seed: 4);
  final anne = _mkVillager('Nur', 'Demirhan', VillagerType.farmer, 14.0, seed: 6);
  final kardes = _mkVillager('Elif', 'Demirhan', VillagerType.priest, 5.0, seed: 9);
  final cocuk = _mkVillager('Deniz', 'Demirhan', VillagerType.farmer, 0.6, seed: 3);
  v.parents.add(anne);
...
List<VillagerStatRow> _statRows() {
  ...
  int tier(double w) => w > 120 ? 4 : w > 80 ? 3 : w > 45 ? 2 : w > 20 ? 1 : 0;
  final out = <VillagerStatRow>[];
  for (int i = 0; i < 34; i++) {
    final v = _mkVillager(names[i % names.length], surnames[i % surnames.length],
        types[i % types.length], 2.0 + (i % 12), seed: i + 3);
...
EventOutcome _eventPlain() => EventSystem.events.firstWhere((e) => e.choices == null);
EventOutcome _eventChoice() => EventSystem.events.firstWhere((e) => e.choices != null);
...
Widget _label(String text) => Padding(
      ...
      child: Text(text.toUpperCase(),
          style: AppUi.label
              .copyWith(fontSize: 8, color: AppUi.textLo, letterSpacing: 1.4)),
    );
...
List<Shot> buildShots() => <Shot>[
      ...
      Shot(
        ...
        build: () => Scaffold(
          ...
          body: Stack(fit: StackFit.expand, children: [
            SaveSlotsPanel(
                onClose: _noop, onContinue: (_) {}, loader: () async => _slots()),
          ]),
        ),
      ),
      Shot(
      ...
      for (final (i, name) in const [(0, 'Köylüler'), (1, 'Haneler')])
        Shot(
          ...
          group: 'Paneller',
          note: 'Defterin NÜFUS bölümü: sıralı köylü listesi ve hane grupları '
      ...
      for (final (i, name) in const [
        ...
        Shot(
          ...
          group: 'Yönetişim',
          note: 'Köy içi işlerin tek kapısı: divan/kanunname/nüfus/tüzük/kronik '
      ...
      Shot(
        ...
        group: 'Yönetişim',
        note: 'İki eksen / dört rejim; mühür öncesi "ibre nereye kayar" önizlemesi.',
        w: 560,
        h: 940,
        settleMs: 1800,
        build: () => Scaffold(
                      ...
                      Expanded(
                        child: ClipRRect(
                          ...
                          child: PetitionSceneCard.custom(
                              scene: s, tone: PetitionTone.solemn, height: 200),
                        ),
                      ),
      ...
      Shot(
        ...
        note: 'Panel, buton türleri, çip, istatistik çubuğu, sekme, yaldızlı çerçeve.',
        w: 1100,
        h: 940,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text('Cinzel başlık / Spectral gövde, soğuk grafit yüzey + ember vurgu',
                style: AppUi.body),
            const AppDivider(),
            ...
            Wrap(spacing: 8, runSpacing: 8, children: const [
              AppChip(label: 'Bereketli', icon: GameIconData.wheat, color: AppUi.sage),
              AppChip(label: 'Kıtlık', icon: GameIconData.warehouse, color: AppUi.rust),
              AppChip(label: 'Ticaret', icon: GameIconData.coin, color: AppUi.gold, solid: true),
              AppChip(label: 'Yağmur', icon: GameIconData.rain, color: AppUi.info),
            ]),
            const SizedBox(height: 14),
            const AppStatBar(
                label: 'Moral', value: 0.63, trailing: '%63', color: AppUi.accent),
            const SizedBox(height: 6),
            const AppStatBar(
                label: 'Ambar', value: 0.37, trailing: '74/200', color: AppUi.sage),
            const SizedBox(height: 6),
            const AppStatBar(
                label: 'Su', value: 0.18, trailing: 'az', color: AppUi.info),
            const AppDivider(),
...
Future<void> main() async {
  ...
  final only = (Platform.environment['ONLY'] ?? '')
      ...
      .toSet();
  final shots = only.isEmpty ? all : all.where((s) => only.contains(s.id)).toList();
  ...
  for (int i = 0; i < shots.length; i++) {
    ...
    stdout.writeln('[${i + 1}/${shots.length}] '
  ...
  stdout.writeln('GALLERY_DONE: ${captured.length}/${shots.length} çekildi, '
