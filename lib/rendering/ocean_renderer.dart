...
class OceanRenderer {
  ...
  static void draw(
    ...
    _seaBands(canvas, w, h, time, lit);
    _sky(canvas, w, horizonY, lit, skyMid, sunColor);
    _horizonHaze(canvas, w, horizonY, lit, sunColor);
    _celestialDisc(
        canvas, size, horizonY, timeOfDay, sunColor, sunOpacity, moonOpacity);
    _clouds(canvas, w, horizonY, time, dayLight, sunColor);
    _reflection(
        canvas, size, horizonY, time, timeOfDay, sunColor, sunOpacity, moonOpacity);
  }
  ...
  static void _sea(Canvas c, double w, double h, double lit, Color skyMid) {
    final seaTop = _scale(Color.lerp(const Color(0xFF4E93B4), skyMid, 0.24)!, lit);
    ...
    _p.shader = ui.Gradient.linear(
        ...
        [seaTop, seaMid, seaBot], const [0.0, 0.45, 1.0]);
  ...
  static void _seaBands(Canvas c, double w, double h, double time, double lit) {
    for (int i = 0; i < 3; i++) {
      ...
      _p.shader = ui.Gradient.linear(
          Offset(0, cy - band), Offset(0, cy + band),
          [
  ...
  static void _sky(Canvas c, double w, double horizonY, double lit, Color skyMid,
      Color sunColor) {
  ...
  static void _horizonHaze(
      Canvas c, double w, double horizonY, double lit, Color sunColor) {
    ...
    _p.shader = ui.Gradient.linear(
        Offset(0, hy - band), Offset(0, hy + band),
        [
  ...
  static void _celestialDisc(Canvas c, Size size, double horizonY,
      double timeOfDay, Color sunColor, double sunOpacity, double moonOpacity) {
    final w = size.width;
    ...
    if (moonOpacity > 0.01) {
      ...
      disc(norm, const Color(0xFFF2F4FF), const Color(0xFFCFE0FF),
          moonOpacity * 0.9, 20);
    }
  }
  ...
  static void _clouds(Canvas c, double w, double horizonY, double time,
      double dayLight, Color sunColor) {
    final cloudA = (0.30 * (0.42 + dayLight * 0.58)).clamp(0.0, 0.40);
    if (cloudA < 0.03) return;
    final base = Color.lerp(const Color(0xFFB8C6D2), Colors.white, dayLight * 0.7)!;
  ...
  static void _puff(
      Canvas c, double cx, double cy, double r, Color tint, double a) {
  ...
  static void _reflection(Canvas c, Size size, double horizonY, double time,
      double timeOfDay, Color sunColor, double sunOpacity, double moonOpacity) {
    final w = size.width, h = size.height;
